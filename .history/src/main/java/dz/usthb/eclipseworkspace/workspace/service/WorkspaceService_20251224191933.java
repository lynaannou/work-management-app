...
public WorkspaceDashboard getDashboardForCurrentUser(int workspaceId) throws SQLException {
    ...
    Workspace workspace = daoWorkspace.findById(workspaceId)
            .orElseThrow(() -> new SQLException("Workspace not found"));

    // 🔐 membership check
    boolean allowed = daoWorkspace.isUserInWorkspace(
            currentUserId,
            workspaceId
    );

    if (!allowed && !security.isLead()) {
        throw new SecurityException("Access denied to this workspace");
    }

    // =============================
    // SAFE BUSINESS LOGIC BELOW
    // =============================
